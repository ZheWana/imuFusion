...
FusionStatus Fusion_MahonyUpdate_6axis(fusion_t *fusionData, bias_t *bias) {
    ...
    fusionData->q0 += 0.5f * (-fusionData->q1 * fusionData->Gx - fusionData->q2 * fusionData->Gy -
                              fusionData->q3 * fusionData->Gz) * fusionData->delta;
    fusionData->q1 += 0.5f * (fusionData->q0 * fusionData->Gx + fusionData->q2 * fusionData->Gz -
                              fusionData->q3 * fusionData->Gy) * fusionData->delta;
    fusionData->q2 += 0.5f * (fusionData->q0 * fusionData->Gy - fusionData->q1 * fusionData->Gz +
                              fusionData->q3 * fusionData->Gx) * fusionData->delta;
    fusionData->q3 += 0.5f * (fusionData->q0 * fusionData->Gz + fusionData->q1 * fusionData->Gy -
                              fusionData->q2 * fusionData->Gx) * fusionData->delta;
...
FusionStatus Fusion_MahonyUpdate_9axis(fusion_t *fusionData, bias_t *bias) {
    Fusion_DataCorrect(fusionData, bias);
    static float vx, vy, vz;
    static float aex, aey, aez;
    ...
    if (fusionData->Mx == 0 && fusionData->My == 0 && fusionData->Mz == 0) {
        ...
        recipNorm = inv_sqrt(fusionData->Mx * fusionData->Mx +
                             fusionData->My * fusionData->My +
                             fusionData->Mz * fusionData->Mz);
        fusionData->Mx *= recipNorm;
        fusionData->My *= recipNorm;
        fusionData->Mz *= recipNorm;

        hx = 2.0f * (fusionData->Mx * (0.5f - q2q2 - q3q3) +
                     fusionData->My * (q1q2 - q0q3) +
                     fusionData->Mz * (q1q3 + q0q2));
        hy = 2.0f * (fusionData->Mx * (q1q2 + q0q3) +
                     fusionData->My * (0.5f - q1q1 - q3q3) +
                     fusionData->Mz * (q2q3 - q0q1));
        ...
        bz = 2.0f * (fusionData->Mx * (q1q3 - q0q2) + // bz = hz
                     fusionData->My * (q2q3 + q0q1) +
                     fusionData->Mz * (0.5f - q1q1 - q2q2));
...
FusionStatus Fusion_KalmanUpdate_6axis(fusion_t *fusionData) {
    ;
}

FusionStatus Fusion_KalmanUpdate_9axis(fusion_t *fusionData) {
    ;
}
